...
(function(a) { if (typeof define === 'function' && define.amd) { define(['jquery'], a); } else if (typeof exports === 'object') { a(require('jquery')); } else { a(jQuery); } }((d, f) => {
            if (!('indexOf' in Array.prototype)) { Array.prototype.indexOf = function(k, j) { if (j === f) { j = 0; } if (j < 0) { j += this.length; } if (j < 0) { j = 0; } for (let l = this.length; j < l; j++) { if (j in this && this[j] === k) { return j; } } return -1; }; }
            ...
            function a() {
                ...
                if (p instanceof Array) {
                    n = [];
                    for (var l = 0, j = p.length; l < j; l++) { o = p[l]; if ((q = (m = o.match(/\b[A-Z]+\b/)) !== null) ? m[0] : 0) { n.push(q); } }
                    p = n.pop();
                }
                return p;
            }

            function h() { return new Date(Date.UTC.apply(Date, arguments)); }
            const g = function(k, j) {
                const m = this;
                this.element = d(k);
                ...
                if ('formatViewType' in j) { this.formatViewType = j.formatViewType; } else if ('formatViewType' in this.element.data()) { this.formatViewType = this.element.data('formatViewType'); }
                this.minView = 0;
                if ('minView' in j) { this.minView = j.minView; } else if ('minView' in this.element.data()) { this.minView = this.element.data('min-view'); }
                this.minView = c.convertViewMode(this.minView);
                this.maxView = c.modes.length - 1;
                if ('maxView' in j) { this.maxView = j.maxView; } else if ('maxView' in this.element.data()) { this.maxView = this.element.data('max-view'); }
                this.maxView = c.convertViewMode(this.maxView);
                this.wheelViewModeNavigation = false;
                if ('wheelViewModeNavigation' in j) { this.wheelViewModeNavigation = j.wheelViewModeNavigation; } else if ('wheelViewModeNavigation' in this.element.data()) { this.wheelViewModeNavigation = this.element.data('view-mode-wheel-navigation'); }
                this.wheelViewModeNavigationInverseDirection = false;
                if ('wheelViewModeNavigationInverseDirection' in j) { this.wheelViewModeNavigationInverseDirection = j.wheelViewModeNavigationInverseDirection; } else if ('wheelViewModeNavigationInverseDirection' in this.element.data()) { this.wheelViewModeNavigationInverseDirection = this.element.data('view-mode-wheel-navigation-inverse-dir'); }
                this.wheelViewModeNavigationDelay = 100;
                if ('wheelViewModeNavigationDelay' in j) { this.wheelViewModeNavigationDelay = j.wheelViewModeNavigationDelay; } else if ('wheelViewModeNavigationDelay' in this.element.data()) { this.wheelViewModeNavigationDelay = this.element.data('view-mode-wheel-navigation-delay'); }
                this.startViewMode = 2;
                if ('startView' in j) { this.startViewMode = j.startView; } else if ('startView' in this.element.data()) { this.startViewMode = this.element.data('start-view'); }
                this.startViewMode = c.convertViewMode(this.startViewMode);
                this.viewMode = this.startViewMode;
                this.viewSelect = this.minView;
                if ('viewSelect' in j) { this.viewSelect = j.viewSelect; } else if ('viewSelect' in this.element.data()) { this.viewSelect = this.element.data('view-select'); }
                this.viewSelect = c.convertViewMode(this.viewSelect);
                this.forceParse = true;
                if ('forceParse' in j) { this.forceParse = j.forceParse; } else if ('dateForceParse' in this.element.data()) { this.forceParse = this.element.data('date-force-parse'); }
                let l = this.bootcssVer === 3 ? c.templateV3 : c.template;
                ...
                this.picker = d(l).appendTo(this.isInline ? this.element : this.container).on({ click: d.proxy(this.click, this), mousedown: d.proxy(this.mousedown, this) });
                if (this.wheelViewModeNavigation) { if (d.fn.mousewheel) { this.picker.on({ mousewheel: d.proxy(this.mousewheel, this) }); } else { console.log('Mouse Wheel event is not supported. Please include the jQuery Mouse Wheel plugin before enabling this option'); } }
                ...
                if (this.isRTL) {
                    ...
                    const i = this.bootcssVer === 3 ? '.prev span, .next span' : '.prev i, .next i';
                ...
                if ('autoclose' in j) { this.autoclose = j.autoclose; } else if ('dateAutoclose' in this.element.data()) { this.autoclose = this.element.data('date-autoclose'); }
                this.keyboardNavigation = true;
                if ('keyboardNavigation' in j) { this.keyboardNavigation = j.keyboardNavigation; } else if ('dateKeyboardNavigation' in this.element.data()) { this.keyboardNavigation = this.element.data('date-keyboard-navigation'); }
                ...
                this.weekStart = 0;
                if (typeof j.weekStart !== 'undefined') { this.weekStart = j.weekStart; } else if (typeof this.element.data('date-weekstart') !== 'undefined') { this.weekStart = this.element.data('date-weekstart'); } else if (typeof e[this.language].weekStart !== 'undefined') { this.weekStart = e[this.language].weekStart; }
                this.weekStart = this.weekStart % 7;
                this.weekEnd = ((this.weekStart + 6) % 7);
                this.onRenderDay = function(n) { let p = (j.onRenderDay || function() { return []; })(n); if (typeof p === 'string') { p = [p]; } const o = ['day']; return o.concat((p || [])); };
                this.onRenderHour = function(n) { let p = (j.onRenderHour || function() { return []; })(n); const o = ['hour']; if (typeof p === 'string') { p = [p]; } return o.concat((p || [])); };
                this.onRenderMinute = function(n) { let p = (j.onRenderMinute || function() { return []; })(n); const o = ['minute']; if (typeof p === 'string') { p = [p]; } if (n < this.startDate || n > this.endDate) { o.push('disabled'); } else if (Math.floor(this.date.getUTCMinutes() / this.minuteStep) === Math.floor(n.getUTCMinutes() / this.minuteStep)) { o.push('active'); } return o.concat((p || [])); };
                this.onRenderYear = function(o) { let q = (j.onRenderYear || function() { return []; })(o); const p = ['year']; if (typeof q === 'string') { q = [q]; } if (this.date.getUTCFullYear() === o.getUTCFullYear()) { p.push('active'); } const n = o.getUTCFullYear(); const r = this.endDate.getUTCFullYear(); if (o < this.startDate || n > r) { p.push('disabled'); } return p.concat((q || [])); };
                this.onRenderMonth = function(n) { let p = (j.onRenderMonth || function() { return []; })(n); const o = ['month']; if (typeof p === 'string') { p = [p]; } return o.concat((p || [])); };
            ...
            g.prototype = {
                    ...
                    _attachEvents() {
                        ...
                        if (this.isInput) {
                            ...
                            if (this.componentReset) { this._events.push([this.componentReset, { click: d.proxy(this.reset, this) }]); }
                        } else if (this.element.is('div')) { this.isInline = true; } else {
                            this._events = [
                    ...
                    setValue() {
                        const i = this.getFormattedDate();
                        if (!this.isInput) {
                            if (this.component) { this.element.find('input').val(i); }
                            this.element.data('date', i);
                        } else { this.element.val(i); }
                        if (this.linkField) { d(`#${this.linkField}`).val(this.getFormattedDate(this.linkFormat)); }
                    },
                    ...
                    setStartDate(i) {
                        ...
                        if (this.startDate.valueOf() !== 8639968443048000) { this.startDate = c.parseDate(this.startDate, this.format, this.language, this.formatType, this.timezone); }
                        this.update();
                        this.updateNavArrows();
                    },
                    setEndDate(i) {
                        this.endDate = i || this.endDate;
                        if (this.endDate.valueOf() !== 8639968443048000) { this.endDate = c.parseDate(this.endDate, this.format, this.language, this.formatType, this.timezone); }
                        this.update();
                        this.updateNavArrows();
                    },
                    setDatesDisabled(j) {
                        this.datesDisabled = j || [];
                        if (!d.isArray(this.datesDisabled)) { this.datesDisabled = this.datesDisabled.split(/,\s*/); }
                        const i = this;
                        this.datesDisabled = d.map(this.datesDisabled, (k) => c.parseDate(k, i.format, i.language, i.formatType, i.timezone).toDateString());
                        this.update();
                        this.updateNavArrows();
                    },
                    setTitle(i, j) { return this.picker.find(i).find('th:eq(1)').text(this.title === false ? j : this.title); },
                    setDaysOfWeekDisabled(i) {
                        this.daysOfWeekDisabled = i || [];
                        if (!d.isArray(this.daysOfWeekDisabled)) { this.daysOfWeekDisabled = this.daysOfWeekDisabled.split(/,\s*/); }
                        this.daysOfWeekDisabled = d.map(this.daysOfWeekDisabled, (j) => parseInt(j, 10));
                        this.update();
                        this.updateNavArrows();
                    },
                    setMinutesDisabled(i) {
                        this.minutesDisabled = i || [];
                        if (!d.isArray(this.minutesDisabled)) { this.minutesDisabled = this.minutesDisabled.split(/,\s*/); }
                        this.minutesDisabled = d.map(this.minutesDisabled, (j) => parseInt(j, 10));
                        this.update();
                        this.updateNavArrows();
                    },
                    setHoursDisabled(i) {
                        this.hoursDisabled = i || [];
                        if (!d.isArray(this.hoursDisabled)) { this.hoursDisabled = this.hoursDisabled.split(/,\s*/); }
                        this.hoursDisabled = d.map(this.hoursDisabled, (j) => parseInt(j, 10));
                        this.update();
                        this.updateNavArrows();
                    },
                    place() {
                        ...
                        if (!this.zIndex) {
                            let j = 0;
                            d('div').each(function() { const o = parseInt(d(this).css('zIndex'), 10); if (o > j) { j = o; } });
                            this.zIndex = j + 10;
                        }
                        let n;
                        let m;
                        let l;
                        let k;
                        if (this.container instanceof d) { k = this.container.offset(); } else { k = d(this.container).offset(); }
                        if (this.component) {
                            n = this.component.offset();
                            l = n.left;
                            if (this.pickerPosition === 'bottom-left' || this.pickerPosition === 'top-left') { l += this.component.outerWidth() - this.picker.outerWidth(); }
                        } else {
                            n = this.element.offset();
                            l = n.left;
                            if (this.pickerPosition === 'bottom-left' || this.pickerPosition === 'top-left') { l += this.element.outerWidth() - this.picker.outerWidth(); }
                        }
                        const i = document.body.clientWidth || window.innerWidth;
                        if (l + 220 > i) { l = i - 220; }
                    ...
                    update() {
                        ...
                        if (arguments && arguments.length && (typeof arguments[0] === 'string' || arguments[0] instanceof Date)) {
                            i = arguments[0];
                            j = true;
                        } else { i = (this.isInput ? this.element.val() : this.element.find('input').val()) || this.element.data('date') || this.initialDate; if (typeof i === 'string') { i = i.replace(/^\s+|\s+$/g, ''); } }
                        if (!i) {
                            i = new Date();
                            j = false;
                        }
                        if (typeof i === 'string') { if (new RegExp(this.hour_minute).test(i) || new RegExp(`${this.hour_minute}:[0-5][0-9]`).test(i)) { i = this.getDate(); } }
                    ...
                    fillDow() {
                        ...
                        let j = '<tr>';
                        while (i < this.weekStart + 7) { j += `<th class="dow">${e[this.language].daysMin[(i++) % 7]}</th>`; }
                        j += '</tr>';
                        this.picker.find('.datetimepicker-days thead').append(j);
                    },
                    fillMonths() {
                        let l = '';
                        const m = new Date(this.viewDate);
                        for (let k = 0; k < 12; k++) {
                            m.setUTCMonth(k);
                            const j = this.onRenderMonth(m);
                            l += `<span class="${j.join(' ')}">${e[this.language].monthsShort[k]}</span>`;
                        }
                        this.picker.find('.datetimepicker-months td').html(l);
                    },
                    fill() {
                        if (!this.date || !this.viewDate) { return; }
                        let E = new Date(this.viewDate);
                        let t = E.getUTCFullYear();
                        const G = E.getUTCMonth();
                        const n = E.getUTCDate();
                        const A = E.getUTCHours();
                        const w = this.startDate.getUTCFullYear();
                        const B = this.startDate.getUTCMonth();
                        const p = this.endDate.getUTCFullYear();
                        const x = this.endDate.getUTCMonth() + 1;
                        const q = (new h(this.date.getUTCFullYear(), this.date.getUTCMonth(), this.date.getUTCDate())).valueOf();
                        const D = new Date();
                        this.setTitle('.datetimepicker-days', `${e[this.language].months[G]} ${t}`);
                        if (this.formatViewType === 'time') {
                            const k = this.getFormattedDate();
                        ...
                        this.updateNavArrows();
                        this.fillMonths();
                        const I = h(t, G - 1, 28, 0, 0, 0, 0);
                        const z = c.getDaysInMonth(I.getUTCFullYear(), I.getUTCMonth());
                        I.setUTCDate(z);
                        ...
                        let F;
                        while (I.valueOf() < j) {
                            if (I.getUTCDay() === this.weekStart) { r.push('<tr>'); }
                            F = this.onRenderDay(I);
                            if (I.getUTCFullYear() < t || (I.getUTCFullYear() === t && I.getUTCMonth() < G)) { F.push('old'); } else if (I.getUTCFullYear() > t || (I.getUTCFullYear() === t && I.getUTCMonth() > G)) { F.push('new'); }
                            if (this.todayHighlight && I.getUTCFullYear() === D.getFullYear() && I.getUTCMonth() === D.getMonth() && I.getUTCDate() === D.getDate()) { F.push('today'); }
                            if (I.valueOf() === q) { F.push('active'); }
                            if ((I.valueOf() + 86400000) <= this.startDate || I.valueOf() > this.endDate || d.inArray(I.getUTCDay(), this.daysOfWeekDisabled) !== -1 || d.inArray(I.toDateString(), this.datesDisabled) !== -1) { F.push('disabled'); }
                            r.push(`<td class="${F.join(' ')}">${I.getUTCDate()}</td>`);
                            if (I.getUTCDay() === this.weekEnd) { r.push('</tr>'); }
                        ...
                        C = '';
                        s = '';
                        const m = this.minutesDisabled || [];
                        E = new Date(this.viewDate);
                        for (var y = 0; y < 60; y += this.minuteStep) {
                            if (m.indexOf(y) !== -1) { continue; }
                            E.setUTCMinutes(y);
                            E.setUTCSeconds(0);
